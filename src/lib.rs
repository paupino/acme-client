...
impl Directory {
    ...
    fn get_nonce(&self) -> Result<String> {
        ...
        res.headers()
            .get("Replay-Nonce")
            .ok_or(ErrorKind::Msg("Replay-Nonce header not found".into()).into())
            .and_then(|nonce| nonce.to_str().map_err(|_| ErrorKind::Msg("Nonce header value contains invalid characters".into()).into()))
